...
export const borrowBook = async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).send({ message: 'User not found' }).populate('books.book', 'name')

        const book = await Book.findById(data.book)
        if (!book) return res.status(404).send({ message: 'Book not found' })

        if (user.books.length >= 2) return res.status(400).send({ message: 'You can not borrow more books'})

        if (user.books.includes(data.book)) return res.status(400).send({ message: 'User has already borrowed this book' })

        user.books.push(data.book)
        await user.save()

        return res.send({ message: 'Book borrowed successfully' })
    } catch (err) {
        console.error(err)
        return res.status(500).send({ message: 'Error borrowing a book' })
    }
}
